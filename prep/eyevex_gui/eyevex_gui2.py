...
Graphical user interface for active_vision/prep/eyevex2.py

Written by Junji Ito (j.ito@fz-juelich.de) on 2016.07.30
"""
import os
import json
import re
...
class ControlPanel(wx.Panel):
    ...
    def Layout(self):
        ...
        # hbox_tmp = wx.BoxSizer(wx.HORIZONTAL)
        # hbox_tmp.Add(wx.StaticText(self, -1, 'S-G filter window length:'), flag=wx.ALIGN_CENTER_VERTICAL)
        # hbox_tmp.Add(self.tc['savgol_window_length'])
        # hbox_tmp.Add(wx.StaticText(self, -1, 'samples ({} Hz sampling rate)'.format(self.Fs)), flag=wx.ALIGN_CENTER_VERTICAL)
        # vbox2.Add(hbox_tmp)
        #
        # hbox_tmp = wx.BoxSizer(wx.HORIZONTAL)
        # hbox_tmp.Add(wx.StaticText(self, -1, 'S-G filter polynomial order:'), flag=wx.ALIGN_CENTER_VERTICAL)
        # hbox_tmp.Add(self.tc['savgol_polyorder'])
        # vbox2.Add(hbox_tmp)
    ...
    def OnDraw(self, event, t_mark=None):
        ...
        # extract eye events
        sac, fix, eyepos, eyevelo, eyeaccl = eyevex2.main(eyecoil, self.Fs, self.transform, self.param['eex'], ret_eyepos=True)
    ...
    def OnApply(self, event):
        ...
        eyecoil = eyecoil.T
        sac, fix = eyevex2.main(eyecoil, self.Fs, self.transform, self.param['eex'], verbose=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data", nargs=4, default=None)
    ...
    parser.add_argument("--calib_method", dest="calibmeth", default=conf['eyevex_gui3']['calib_method'])
    parser.add_argument("--calib_param", dest="calibparam", default=conf['eyevex_gui3']['calib_param'])
    parser.add_argument("--calib_ignore", dest="calibignore", nargs='*', type=int, default=[-1,])
    parser.add_argument("--on_event", default=conf['eyevex_gui3']['on_event'])
    parser.add_argument("--off_event", default=conf['eyevex_gui3']['off_event'])
    ...
    datadir_sbj = "{}/{}".format(datadir, sbj)
    calibblk = int(calibblk)
        
    # load metadata parameters from odML file
    param = load_odml(datadir_sbj, sess, rec, blk)
    
    # add parameters from command line arguments
    param.update(
        {'datadir': datadir_sbj, 'sess': sess, 'rec': rec, 'blk': blk,
         'stimdir': arg.stimdir, 'on_event': arg.on_event,
         'off_event': arg.off_event,})
    
    # load eye event extraction parameters from configuration file, if they are
    # not found in odML
    if 'eex' not in param:
        param['eex'] = conf['eyevex_gui3']['eex_param']
        
    # load calibration parameters
    param['calib'] = load_calibparam(datadir_sbj, calibsess, calibrec, calibblk)
    ...
    title = "EYEVEX: Eye Event Extractor ({}:{}_rec{}_blk{})".format(sbj, sess, rec, blk)
